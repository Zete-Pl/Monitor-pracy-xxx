const Calculations = {
  ...
  getMonthSettings: (state, month = null) => {
    ...
    return monthSettings[selectedMonth] || { persons: {}, clients: {}, settlementConfig: {}, personContractCharges: {} };
  },
  ...
  buildInvoiceClientScopedState: (state, clientSource, month = null, options = {}) => {
    ...
    (state?.expenses || []).forEach(expense => {
      ...
      if (expense?.type === 'BONUS') {
        ...
        scopedExpenses.push({
          ...expense,
          id: `${expense.id}::client-bonus::${clientId || normalizedClientName}`,
          amount
        });
        return;
      }
  ...
  calculateInvoices: (state, month = null, options = {}) => {
    ...
    const issuerTotals = {};
    const issuerYearToDateBaseTotals = {};
    ...
    if (includeYearToDate && /^\d{4}-\d{2}$/.test(selectedMonth)) {
      const [selectedYear, selectedMonthNumber] = selectedMonth.split('-').map(Number);
      for (let monthNumber = 1; monthNumber < selectedMonthNumber; monthNumber++) {
        const monthKey = `${selectedYear}-${String(monthNumber).padStart(2, '0')}`;
        const monthInvoices = Calculations.calculateInvoices(state, monthKey, { includeYearToDate: false });
        (monthInvoices.issuerSummaries || []).forEach(summary => {
          if (summary.issuerType !== 'PARTNER' && summary.issuerType !== 'WORKING_PARTNER') return;
          issuerYearToDateBaseTotals[summary.issuerId] = (issuerYearToDateBaseTotals[summary.issuerId] || 0) + (summary.totalAmount || 0);
        });
      }
    }
  ...
  getSettlementInvoiceParticipants: (settlement) => {
    ...
    return entries
      ...
      .map(entry => {
        const gross = Math.max(0, parseFloat(entry.toPayout) || 0);
        const grossWithEmployees = Math.max(0, Number.isFinite(parseFloat(entry.grossWithEmployeeSalaries))
  ...
  calculateInvoiceTaxEqualization: (state, settlement, month = null) => {
    ...
    const targetTaxByPerson = Object.fromEntries(participants.map(participant => [participant.personId, parseFloat(settlementEntryByPersonId[participant.personId]?.taxAmount) || 0]));
  ...
  calculateInvoiceReconciliation: (state, month = null) => {
    ...
    const revenuePayers = issuers
      .map(entry => ({
        ...
        remainingAmount: Math.max(0, (entry.revenueBalance || 0) - (entry.revenueTargetAmount || 0))
      }))
      .filter(entry => entry.remainingAmount >= 0.005);
    ...
    issuers.forEach(entry => {
      ...
      entry.postTaxTargetAmount = entry.issuerType === 'SEPARATE_COMPANY'
        ? Math.max(0, (entry.grossTargetAmount || 0) - outgoingTaxReimbursements)
        : Math.max(0, (entry.grossTargetAmount || 0) - (entry.targetTaxAmount || 0) - (entry.ownZusAmount || 0));
    });
  ...
  generateSettlement: (state, options = {}) => {
    ...
    const employeeBonusByRecipientId = selectedExpenses
      ...
      .reduce((totals, expense) => {
        totals[expense.advanceForId] = (totals[expense.advanceForId] || 0) + (parseFloat(expense.amount) || 0);
        return totals;
      }, {});
    ...
    const processedEmployees = empStats.map(emp => {
      ...
      return {
        ...
        toPayout: emp.salary + bonusAmount + paidCosts + paidAdvances - advancesTaken - deductedContractTaxAmount - deductedContractZusAmount
      };
    });
    ...
    const processedWorkingPartnersBase = workingPartnerStats.map(wp => {
      ...
      const grossBeforeAccounting = wp.salary + worksShare + paidCosts + paidAdvances - appliedCostShare - advancesTaken;
    ...
    processedEmployees.forEach(employee => {
      ...
      const adjustedProfitContribution = profitContribution - (employee.bonusAmount || 0);
    ...
    let processedPartners = partnerStats.map(p => {
      ...
      const employerEmployeeTotals = employerEmployeeSalaryByEmployer[p.person.id] || { positive: 0, receivable: 0, signed: 0, accountingRefund: 0 };
      ...
      const toPayout = ownGrossAmount + paidCosts + paidAdvances - appliedCostShare - advancesTaken;
    ...
    const processedSeparateCompanies = separateCompanyStats.map(company => {
      ...
      const totalRevenueShare = revenueShare + worksShare;
      const employerEmployeeTotals = employerEmployeeSalaryByEmployer[company.person.id] || { positive: 0, receivable: 0, signed: 0, accountingRefund: 0, bonuses: 0 };
      const employeeSalaries = employerEmployeeTotals.positive;
      const employeeReceivables = employerEmployeeTotals.receivable;
      const employeeAccountingRefund = Calculations.countsEmployeeAccountingRefund(company.person)
        ...
        : 0;
      const employeeBonuses = employerEmployeeTotals.bonuses || 0;
      const toPayout = company.salary + totalRevenueShare + paidCosts + paidAdvances - appliedCostShare - advancesTaken;
    ...
    let processedWorkingPartners = processedWorkingPartnersBase.map(wp => {
      const employerEmployeeTotals = employerEmployeeSalaryByEmployer[wp.person.id] || { positive: 0, receivable: 0, signed: 0, accountingRefund: 0, bonuses: 0 };
      const employeeSalaries = employerEmployeeTotals.positive;
      const employeeReceivables = employerEmployeeTotals.receivable;
      const employeeAccountingRefund = employerEmployeeTotals.accountingRefund;
      const employeeBonuses = employerEmployeeTotals.bonuses || 0;
